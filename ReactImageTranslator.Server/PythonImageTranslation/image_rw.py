...
class image_rw:
    ...
    def get_text_boxes(self, image_filepath, source_lang):
        ...
        # read page for boxes
        reader = eocr.Reader([source_abrv], model_storage_directory='PythonImageTranslation/models', user_network_directory='PythonImageTranslation/models', recog_network='japanese_g2')
        #text_boxes_list = reader.readtext(image, paragraph=True, x_ths=.15, y_ths=.15, rotation_info=[90,180,270], add_margin=0.15) #original
        text_boxes_list = reader.readtext(image, paragraph=True, x_ths=.05, y_ths=.5, ycenter_ths=.1, add_margin=0.1, height_ths=.4, min_size=52) 
